...
import { X, FileCode, FileJson, FileType2, Database, Container, Code } from 'lucide-react';
...
const getFileIcon = (name: string) => {
    if (name.endsWith('.php')) return <span className="text-[#8892be] font-bold text-[10px]">PHP</span>;
    if (name.endsWith('.yml')) return <Container size={14} className="text-blue-400" />;
    if (name.endsWith('.xml')) return <Code size={14} className="text-orange-400" />;
...
const highlightSyntax = (code: string) => {
    ...
    return code
        ...
        .replace(/>/g, '&gt;')
        // PHP Keywords
        .replace(/\b(namespace|use|class|public|protected|function|return|new|try|catch|if|throw)\b/g, '<span class="text-[#c678dd]">$1</span>')
        // Magento/PHP Classes
        .replace(/\b([A-Z][a-zA-Z0-9_]*)\b/g, '<span class="text-[#e5c07b]">$1</span>')
        // Variables
        .replace(/(\$[a-zA-Z0-9_]+)/g, '<span class="text-[#e06c75]">$1</span>')
        // YML Keys
        .replace(/^(\s*)([a-z0-9_-]+):/gm, '$1<span class="text-[#e06c75]">$2</span>:')
        // Strings
        ...
        <div className="flex flex-col h-full bg-[#1e1e1e] text-sm font-mono overflow-hidden">
            <div className="flex bg-[#252526] overflow-x-auto no-scrollbar border-b border-[#1e1e1e]">
                ...
                `}
                            >
                                {isActive && (
                                    <motion.div layoutId="activeTabIndicator" className="absolute top-0 left-0 right-0 h-[2px] bg-[#f27036]" /> // Magento Orange
                                )}
                                <span className="shrink-0">{getFileIcon(file.name)}</span>
                                <span className="truncate text-[13px]">{file.name}</span>
                                <button onClick={(e) => onCloseTab(file.id, e)} className={`ml-auto rounded-sm p-0.5 opacity-0 group-hover:opacity-100 hover:bg-slate-700 transition-all ${isActive ? 'opacity-100 text-white' : 'text-slate-400'}`}>
                                    <X size={13} />
                                </button>
                            </motion.div>
                        );
            ...
            <div className="flex-1 relative overflow-hidden bg-[#1e1e1e]">
                <AnimatePresence mode="wait">
                  ...
                  <code dangerouslySetInnerHTML={{ __html: highlightSyntax(activeFile.content || '') }} />
                </pre>
                            </div>
                        </motion.div>
                    ) : (
                        <div className="flex flex-col items-center justify-center h-full text-slate-600 select-none">
                            <div className="w-24 h-24 mb-4 bg-slate-800/50 rounded-full flex items-center justify-center">
                                <span className="text-4xl font-bold text-[#f27036] opacity-30">M2</span>
                            </div>
