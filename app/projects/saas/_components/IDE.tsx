...
export const IDE = () => {
    const initialFile = projectFiles[0]?.children?.[1]?.children?.[0]?.children?.[0]?.children?.[0]?.children?.[0]?.children?.[0]?.children?.[0] || projectFiles[0];

    const [openFiles, setOpenFiles] = useState<FileNode[]>(() => {
        return (initialFile && initialFile.type === 'file') ? [initialFile] : [];
    });

    const [activeFileId, setActiveFileId] = useState<string | null>(() => {
        return (initialFile && initialFile.type === 'file') ? initialFile.id : null;
    });
